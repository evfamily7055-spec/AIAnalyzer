...
st.info("実データをAIに送信せず、拡張スキーマと日本語の指示だけでPandasコードを自動生成し、実行します。")
...
if st.session_state.df is not None:
    ...
    # --- タブ1: データプレビュー (シートのヘッド) ---
    with tab1:
        st.subheader("データの先頭100行")
        st.dataframe(st.session_state.df.head(100), use_container_width=True, height=600)

    # --- タブ2: AIへの指示 ---
    with tab2:
        st.subheader("AIへの集計指示")
        st.write("隣の「データプレビュー」タブで列名を確認しながら、実行したい集計を日本語で指示してください。")
            ...
            if not api_key:
                ...
                with st.spinner("AIがPandasコードを生成中です..."):
                    ...
                    if generated_code:
                        ...
                        st.success("コードが生成されました。「生成されたコード」タブで確認・実行してください。")

    # --- タブ3: 生成されたコード ---
    with tab3:
        st.subheader("AIが生成したPandasコード")
        if st.session_state.generated_code:
                ...
                with st.spinner("サーバー上でコードを実行中..."):
                    try:
                        ...
                        if result is not None:
                            ...
                            st.success("コードが実行されました。「実行結果」タブで確認してください。")
                        else:
            ...
            st.info("「AIへの指示」タブでコードを生成してください。")

    # --- タブ4: 実行結果 ---
    with tab4:
        st.subheader("集計・分析結果")
        if st.session_state.exec_result is not None:
            ...
            st.info("「生成されたコード」タブでコードを実行してください。")


    # --- (NEW) 3. 拡張スキーマ (画面下部に折りたたみで配置) ---
    st.markdown("---")
